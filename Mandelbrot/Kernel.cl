//int linearInterpolation(int &colour1[3], int &color2[3], float t, int colour) {
//    if(colour == 0){return color1[0] + (color2[0] - color1[0]) * t;}
//    if(colour == 1){return color1[1] + (color2[1] - color1[1]) * t;}
//    if(colour == 2){return color1[2] + (color2[2] - color1[2]) * t;}
//}
...
if(colouringScheme == 1){
    ...
    while (idx < totalPixels) {
        ...
        if(iteration == maxIterations){
            pixelArr[pixelIndex] = 50; //red
            pixelArr[totalPixels + pixelIndex] = 50; //green
            pixelArr[totalPixels * 2 + pixelIndex] = 150; //blue
        }
        else{
            //u = z/der
